...
[Meet the rest of the Teaching Team](https://stat-100.com/people.html) on the course website. 
...
p_zones <- 
  ...
  geom_rect(
    aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, 
        fill = fill_color, color = border_color, linewidth = border_size),
    show.legend = FALSE
  ) +
  scale_fill_identity() +
  ...
  # Zone titles (Balanced at 12pt)
  geom_text(
    aes(x = (xmin + xmax) / 2, y = 3.15, label = title, color = text_color),
    fontface = "bold", size = 12 / .pt, lineheight = 0.95, show.legend = FALSE
  ) +
  
  # Bulleted details (Scaled to 9.5pt)
  annotate(
    "text", x = 65, y = 1.85, 
    label = z3_details,
    color = "#FFFFFF", fontface = "bold", size = 9.5 / .pt, lineheight = 1.15
  ) +
  
  # Axis line
  geom_segment(
    aes(x = 0, xend = 100, y = 0.35, yend = 0.35),
    arrow = arrow(length = unit(0.25, "cm"), type = "closed"),
    color = "#1E1E1E", linewidth = 0.8
  ) +
  
  # Axis endpoint labels (Scaled to 11pt)
  annotate("text", x = 0, y = -0.1, label = "Low Effort", fontface = "bold", size = 11 / .pt, hjust = 0, color = "#1E1E1E") +
  annotate("text", x = 100, y = -0.1, label = "Excessive Effort / Overload", fontface = "bold", size = 11 / .pt, hjust = 1, color = "#1E1E1E") +
  
  coord_cartesian(xlim = c(-2, 102), ylim = c(-0.4, 4.4), clip = "off") +
  theme_void() +
  theme(plot.margin = margin(10, 10, 10, 10))

# ggsave(here::here("images/lec01b-learning-zones.png"))

knitr::include_graphics(here::here("images/lec01b-learning-zones.png"))
```
...
People played chance games but they "probably" did not yet ask "What are the odds?"
...
plot_probability <- ggplot() +
  ...
  annotate("text", x = 2, y = 4.55, label = expression(pi == 0.5), 
           color = color_text_muted, size = 7, fontface = "bold") +
...
In many instances, we do not know the design of the coin and we are wondering if it is a fair coin. 
...
```{r}
#| echo: false
#| fig-alt: a black eraser with white font that reads errare humanum est
knitr::include_graphics("images/lec01b-eraser.jpg")
```
...
- Mistakes are part of the learning process. "Good mistakes" help you learn. 
