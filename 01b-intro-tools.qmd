...
wrapped_titles <- str_wrap(zone_titles, width = 14)
z3_details <- str_wrap("• High Effort\n• Delayed Gratification\n• Durable Knowledge", width = 18)
...
ggplot(zones) +
  ...
  # Larger relative text size on a 6.5-inch canvas
  geom_text(
    aes(x = (xmin + xmax) / 2, y = 3.1, label = title, color = text_color),
    fontface = "bold", size = 15 / .pt, lineheight = 0.9, show.legend = FALSE
  ) +
  
  annotate(
    "text", x = 65, y = 1.8, 
    label = z3_details,
    color = "#FFFFFF", fontface = "bold", size = 11 / .pt, lineheight = 1.1
  ) +
  
  geom_segment(
    aes(x = 0, xend = 100, y = 0.3, yend = 0.3),
    arrow = arrow(length = unit(0.3, "cm"), type = "closed"),
    color = "#1E1E1E", linewidth = 1
  ) +
  
  annotate("text", x = 0, y = -0.15, label = "Low Effort", fontface = "bold", size = 13 / .pt, hjust = 0, color = "#1E1E1E") +
  annotate("text", x = 100, y = -0.15, label = "Excessive Effort / Overload", fontface = "bold", size = 13 / .pt, hjust = 1, color = "#1E1E1E") +
