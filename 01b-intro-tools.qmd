...
p <- ggplot(zones) +
  ...
  # Zone titles (Scaled to 20pt)
  geom_text(
    aes(x = (xmin + xmax) / 2, y = 3.1, label = title, color = text_color),
    fontface = "bold", size = 20 / .pt, lineheight = 0.9, show.legend = FALSE
  ) +
  
  # Wrapped key attributes in Zone 3 (Scaled to 15pt)
  annotate(
    "text", x = 65, y = 1.8, 
    label = z3_details,
    color = "#FFFFFF", fontface = "bold", size = 15 / .pt, lineheight = 1.1
  ) +
  ...
  geom_segment(
    ...
    color = "#1E1E1E", linewidth = 1.1
  ) +
  
  # Axis endpoint labels (Scaled to 18pt)
  annotate("text", x = 0, y = -0.15, label = "Low Effort", fontface = "bold", size = 18 / .pt, hjust = 0, color = "#1E1E1E") +
  annotate("text", x = 100, y = -0.15, label = "Excessive Effort / Overload", fontface = "bold", size = 18 / .pt, hjust = 1, color = "#1E1E1E") +
